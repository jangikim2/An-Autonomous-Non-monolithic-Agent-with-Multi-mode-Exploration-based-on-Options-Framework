...
def log_video(env_name, actor):
    ...
    step = 1
    state = env.reset()
    ###############jangikim################## frame_buffer = []
    while not done:
        ###############jangikim################## frame_buffer.append(env.render(mode='rgb_array'))
        action = actor(torch.Tensor(state)).detach().cpu()
        state, reward, done, info = env.step(action)
        step += 1
    print('    > Finished collection, saved video.\n')
    ###############jangikim################## frame_buffer = np.array(frame_buffer).transpose(0, 3, 1, 2)
    ###############jangikim################## wandb.log({"video": wandb.Video(frame_buffer, fps=30, format="mp4")})
    env.close()
...
def log_video_hrl(env_name, actor_low, actor_high, params):
    ...
    ###############jangikim################## episode_reward, frame_buffer = 0, []
