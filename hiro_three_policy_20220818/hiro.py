...
def homeostasis(x_t, time_step, x_bar, x_squared_bar, x_plus_bar, rho, target_rate_rho):
...
def train(params):
    ...
    for t in range(step, max_timestep):
        ...
        if (t + 1) % c_m == 0 and t > 0:
            if t >= start_timestep:
                temp_success_ratio_m = episode_done_count_m / episode_count_m
                done_m = (temp_success_ratio_m >= 0.6)
                ##done_m = (temp_success_ratio_m >= 0.3)
                ##print("done_m : ", done_m)
                ##print("episode_reward_h_m.cpu().numpy() : ", episode_reward_h_m[0].cpu().numpy())
                if m_flag == False:
                    tmp_action_m = ppom_agent.select_action(np.concatenate([state.cpu().numpy(), target_pos.cpu().numpy()]))
                    m_flag = True

                if action_m == 0:
                    current_episode_reward_h_m = episode_reward_h_m[0].cpu().numpy()
                    current_episode_reward_h_m = current_episode_reward_h_m + 0.5*current_episode_reward_h_m
                    ppom_agent.buffer.rewards.append(current_episode_reward_h_m)
                elif action_m == 2:
                    current_episode_reward_h_m = episode_reward_h_m[0].cpu().numpy()
                    current_episode_reward_h_m = current_episode_reward_h_m + current_episode_reward_h_m
                    ppom_agent.buffer.rewards.append(current_episode_reward_h_m)
                else:
                    ppom_agent.buffer.rewards.append(episode_reward_h_m[0].cpu().numpy())
                ppom_agent.buffer.is_terminals.append(done_m)
                ppom_agent.buffer.success_rates.append(success_rate)

                # update PPO agent
                if (t + 1) >= (start_timestep + update_timestep_m) and (t+1) % update_timestep_m == 0:
                    ###############################################################################################################
                    errorm_logs = ppom_agent.train()

                    loss = errorm_logs[0]
                    surr_loss = errorm_logs[1]
                    value_est_loss = errorm_logs[2]
                    entropy_loss = errorm_logs[3]

                    ##action_m = action_m * 1000

                    record_logger(args=[loss, surr_loss, value_est_loss, entropy_loss, on_policy_count, off_policy_count, random_policy_count], option='ppom_high', step=t-start_timestep)
                    ###############################################################################################################
        ...
        if (t + 1) % c == 0 and t > 0:
            ...
            if t < start_timestep:
                ...
                ##print("episode_reward_h.cpu().numpy() : ", episode_reward_h[0].cpu().numpy())
                ##print("state.cpu().numpy() : ", state.cpu().numpy())
                ##print("target_pos.cpu().numpy() : ", target_pos.cpu().numpy())
                if action_m == 0:
                    if ppo_flag == False:
                        temp_next_goal = ppo_agent.select_action(np.concatenate([state.cpu().numpy(), target_pos.cpu().numpy()]))
                        ppo_flag = True
                    # saving reward and is_terminals
                    ppo_agent.buffer.rewards.append(episode_reward_h[0].cpu().numpy())
                    ppo_agent.buffer.is_terminals.append(temp_done_h)

                    if (t + 1) >= (start_timestep + update_timestep) and (t + 1) % update_timestep == 0:
                        ##############################################################################################################
                        error_logs = ppo_agent.train()

                        loss = error_logs[0]
                        surr_loss = error_logs[1]
                        value_est_loss = error_logs[2]
                        entropy_loss  = error_logs[3]

                        record_logger(args=[loss, surr_loss, value_est_loss, entropy_loss], option='ppo_high', step=t-start_timestep)

                        ###############################################################################################################

                    next_goal = ppo_agent.select_action(np.concatenate([next_state.cpu().numpy(), target_pos.cpu().numpy()]))
                    next_goal = Tensor(next_goal).to(device)
                    next_goal = torch.min(torch.max(next_goal, -max_goal), max_goal)
                    ...
                    temp_next_goal = ppo_agent.select_action(np.concatenate([next_state.cpu().numpy(), target_pos.cpu().numpy()]))
                ...
                ##if t < (start_timestep + update_timestep):
                ##    ppo_agent.buffer.clear()
                '''
                temp_next_state_pos = (next_state, target_pos)
                logprobs, state_values, dist_entropy = ppo_agent.policy.evaluate(torch.cat(temp_next_state_pos), next_goal)
                previous_state_values = state_values.detach().cpu().numpy()[0]
                state_values_list.append(previous_state_values)
                ...
                previous_q_target_h = q_target_h.detach().cpu().numpy()[0][0]
                q_target_h_list.append(previous_q_target_h)
                '''
        ...
        if t >= start_timestep:
            ...
            '''
            temp_episode_reward_l = gamma_tigeer_l * intri_reward.detach().cpu().numpy()
            value_l_reward += temp_episode_reward_l
            gamma_tigeer_l = gamma_tigeer_l * gamma

            policy_params = params.policy_params
            low_next_action = (actor_target_l(next_state, next_goal)).clamp(-policy_params.max_action, policy_params.max_action)
            q_target_1, q_target_2 = critic_target_l(next_state, next_goal, low_next_action)
            q_target_l = torch.min(q_target_1, q_target_2)
            ##q_target_l_list.append(q_target_l.detach().cpu().numpy()[0][0])
            previous_q_target_l = q_target_l.detach().cpu().numpy()[0][0]
            q_target_l_list.append(previous_q_target_l)
            ##q_target_l_var_rms.update(previous_q_target_l)
            '''
